...
# Define the HMM
n_components = 4  # Experiment with the number of hidden states
...
predicted_df['Smoothed'] = predicted_df['Close'].rolling(window=3).mean()  # 3-day moving average

# Calculate Mean Squared Error (if you have actual prices for the next 10 days)
# Here is a dummy creation of actual_prices for demonstration; replace it with your actual data
actual_prices = np.random.normal(loc=predicted_prices[-1], scale=1, size=forecast_days)  # Placeholder
mse = mean_squared_error(actual_prices, predicted_df['Close'])
print(f'Mean Squared Error (MSE) for the predictions: {mse:.4f}')

# Plot the predicted prices for the next 10 days only
plt.figure(figsize=(10, 5))
plt.plot(predicted_df.index, predicted_df['Close'], marker='o', linestyle='-', color='orange', label='Predicted Price')
